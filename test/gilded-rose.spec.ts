...
describe('Gilded Rose', function () {
    ...
    // it should reduce quality of any Conjured item by 2
    it('should reduce quality of any Conjured item by 2', function () {
        const gildedRose = new GildedRose([new Item('Conjured Mana Cake', 3, 33)]);
        const items = gildedRose.updateQuality();
        expect(items[0].quality).to.equal(31);
    });

    // it should reduce quality of any Conjured item by 4 if sellIn <0
    it('should reduce quality of any Conjured item by 4 if sellIn <0', function () {
        const gildedRose = new GildedRose([new Item('Conjured Banana', 0, 25)]);
        const items = gildedRose.updateQuality();
